...
def stations(name,url):
	...
	next = baseurl + str(re.compile('next"><a href="(.+?)"').findall(response)[-1])
	soup = BeautifulSoup(response,'html5lib').find_all('a',{'title':'Play in new window'})
	for item in soup:
	    try: title = re.compile('title=(.+?)&').findall(str(item))[-1]
	    except IndexError:
	        continue
            mount = baseurl + str(re.compile('open\\(\'(.+?)\',').findall(str(item))[-1]).split('&')[0]
	    add_directory2(title,mount,40,defaultfanart,defaultimage,plot='')
