...
def cats(url):
	...
	soup = BeautifulSoup(response,'html5lib').find_all('div',{'class':'views-field views-field-name'})
	for item in soup:
...
def iframe(name, url):
	...
	page = urllib2.urlopen(request).read()
        url = re.compile('"url":"(.+?)"').findall(str(page))[2].replace('json?base64_init=1','m3u8')
	print url
	if url.find(',')!=-1:
	    url = url.split(',', 1)[0]
	    #print url
