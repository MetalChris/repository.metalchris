...
fullbase = 'aHR0cDovL2lsLnNyZ3Nzci5jaC9pbnRlZ3JhdGlvbmxheWVyLzEuMC91ZS9ydHMvdmlkZW8vcGxheS8='
token_url = 'aHR0cDovL3RwLnNyZ3Nzci5jaC9ha2FoZC90b2tlbj9hY2w9JTJGaSUyRnRqX21pZGklMkYq'
live_token = 'aHR0cDovL3RwLnNyZ3Nzci5jaC9ha2FoZC90b2tlbj9hY2w9JTJGaSUyRmVuYzExdW5pX2NoJTQwMTkxNDU1JTJGKg=='
artbase = 'special://home/addons/plugin.video.rtsinfo/resources/media/'
_addon = xbmcaddon.Addon()
_addon_path = _addon.getAddonInfo('path')
selfAddon = xbmcaddon.Addon(id='plugin.video.rtsinfo')
self = xbmcaddon.Addon(id='plugin.video.rtsinfo')
translation = selfAddon.getLocalizedString
usexbmc = selfAddon.getSetting('watchinxbmc')
settings = xbmcaddon.Addon(id="plugin.video.rtsinfo")
...
defaultimage = 'special://home/addons/plugin.video.rtsinfo/icon.png'
defaultfanart = 'special://home/addons/plugin.video.rtsinfo/fanart.jpg'
defaulticon = 'special://home/addons/plugin.video.rtsinfo/icon.png'


local_string = xbmcaddon.Addon(id='plugin.video.rtsinfo').getLocalizedString
...
#633
def sites():
        addDir('RTS Info en continu', 'aHR0cDovL2lsLnNyZ3Nzci5jaC9pbnRlZ3JhdGlvbmxheWVyLzEuMC91ZS9ydHMvdmlkZW8vcGxheS8xOTY3MTI0Lmpzb24=', 50, defaultimage)
        addDir('Journal de 12h45', 'http://www.rts.ch/video/info/journal-12h45/?format=rss/podcast', 634, defaultimage)
        addDir('Journal de 19h30', 'http://www.rts.ch/video/info/journal-19h30/?format=rss/podcast', 634, defaultimage)
...
def videos(url):
	...
	for item in soup:#[1:-1]:
		...
		#print 'utf-8'
	    try: image = re.compile('img src="(.+?)\\?').findall(str(item))[-1]
	    except IndexError:
		continue
	    url = item.find('enclosure')['url']
	    r = requests.head(url)
	    if r.status_code == 404:
		key = item.find('guid').text
		jurl = fullbase.decode('base64') + key + '.json'
		add_directory2(title,jurl,40,defaultfanart,image,plot='')
...
def streams(name,url):
	...
	stream = jdata['Video']['Playlists']['Playlist'][0]['url'][0]['text']
	key = jdata['Video']['AssetSet']['title']
	print key
	if key == '19h30':
	    t_url = token_url.decode('base64').replace('_midi', '')
	else:
...
if mode == None or url == None or len(url) < 1:
	...
	sites()
        ...
        print "RTS - Info Videos"
	videos(url)
elif mode==40:
        print "RTS - Info Streams"
	streams(name,url)
elif mode==50:
        print "RTS Info en continu"
	en_continu(name,url)
