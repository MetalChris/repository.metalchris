...
def how_stuff_works(url):
	...
	soup = BeautifulSoup(response, 'html5lib').find_all("div",{"class":"sidebar col-sm-3 hidden-xs"})[0]
...
def most_watched(name,url):
	...
	if name == 'Most Watched':
		soup = BeautifulSoup(response, 'html5lib').find_all("div",{"class":"module module-multi module-grid most-watched"})[0]
	else:
		soup = BeautifulSoup(response, 'html5lib').find_all("div",{"class":"module module-multi module-grid all-videos"})[0]
