...
def live(name,url,iconimage):
	...
	if str(live_key) != 'False':
	    stream = (jdata['data']['church']['bitrates'][0]['src'])
...
def get_vod(url):
	...
	if 'pagination' in str(html):
	    ...
	    next = int(page) + 1
	    next_url = 'http://www.fcclive.com/sermon-archives/page/' + str(next) +'/'
...
def sermons(url):
	...
	if 'pagination' in str(html):
	    ...
	    page = int(filter(str.isdigit, this_url))
	    next = int(page) + 1
	    next_url = 'http://www.fcclive.com/page/' + str(next) +'/?sermons-category=' + str(category)
	    if str(links[-1]) != str(this_url):
...
if mode == None or url == None or len(url) < 1:
	...
	get_vod(url)
        ...
        print 'FCC VOD CATEGORIES'
	categories(url)
elif mode==15:
        print 'FCC VOD SERMONS'
	sermons(url)
elif mode==30:
        print 'FCC VOD ALL'
	streams(name,url)
