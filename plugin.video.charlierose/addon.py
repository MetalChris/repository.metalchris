...
text = 'Text description of video goes here.'
...
def episodes(url):
	...
	for day, guest, episode in zip(days, guests, episodes):
		...
		add_directory2(title,url,30,defaultfanart,defaultimage,plot=text)
...
def guests(url):
	...
	try:nextp = ('https://charlierose.com' + re.compile('next" href="(.+?)"').findall(response)[-1]).replace('&amp;','&')
	except IndexError:
		return
	print nextp
	add_directory2('Next Page',nextp,16,defaultfanart,defaultimage,plot='')
		#xbmc.executebuiltin("Container.SetViewMode("+str(confluence_views[3])+")")
	xbmcplugin.endOfDirectory(addon_handle)
...
def gstreams(url):
	...
	description = re.compile('<span>(.+?)\\n(.+?)\\n').findall(str(soup))
	plots = (zip(*description)[1])
	for link, date, image, plot in zip(links, dates, images, plots):
		url = 'https://charlierose.com/video/player/' + link.split('/')[-1]
		add_directory2(date,url,30,defaultfanart,image,plot=plot.strip())
		xbmcplugin.setContent(pluginhandle, 'episodes')
		#xbmc.executebuiltin("Container.SetViewMode("+str(confluence_views[3])+")")
	xbmcplugin.endOfDirectory(addon_handle)
...
def cvideos(url):
	...
	for item in soup:
		...
		plot = description.find('b').string.encode('utf-8')
		#duration = description.find('i').string.encode('utf-8')
		image = item.find('img')['src']
		add_directory2(title,url,30,defaultfanart,image,plot=plot)#; i = i + 1
		xbmcplugin.setContent(pluginhandle, 'episodes')
	try:nextp = 'https://charlierose.com' + re.compile('next" href="(.+?)"').findall(response)[-1]
	except IndexError:
		return
	add_directory2('Next Page',nextp,18,defaultfanart,defaultimage,plot='')
		#xbmc.executebuiltin("Container.SetViewMode("+str(confluence_views[3])+")")
	xbmcplugin.endOfDirectory(addon_handle)
...
def videos(url):
	...
	for item in soup:
		...
		add_directory2(title,url,30,defaultfanart,image,plot=plot)#; i = i + 1
		xbmcplugin.setContent(pluginhandle, 'episodes')
		#xbmcplugin.addStreamInfo('video', { 'duration': duration })
	try:nextp = 'https://charlierose.com' + re.compile('next" href="(.+?)"').findall(response)[-1]
	except IndexError:
		return
	add_directory2('Next Page',nextp,20,defaultfanart,defaultimage,plot='')
		#xbmc.executebuiltin("Container.SetViewMode("+str(confluence_views[3])+")")
	xbmcplugin.endOfDirectory(addon_handle)
